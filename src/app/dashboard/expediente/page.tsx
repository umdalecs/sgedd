                ...
                <div className="rounded-2xl border shadow-2xl">
                    <Table>
                        <TableHeader className="sticky top-0 bg-background">
                            <TableRow>
                                <TableHead className="text-center text-gray-500">Documento</TableHead>
                                <TableHead className="text-center text-gray-500">Solicitar</TableHead>
                                <TableHead className="text-center text-gray-500">Estado</TableHead>
                                <TableHead className="text-center text-gray-500">Ultima actualización</TableHead>
                                <TableHead className="text-center text-gray-500">Acción</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {documentos.map((doc) => (
                                <TableRow key={doc.id}>
                                    ...
                                    <TableCell className="text-center">
                                        {doc.solicitar === true ? (
                                        <Button variant="default" size="icon" className="rounded-2xl w-3/5">
                                            <FilePlus className="h-4 w-4" />
                                        </Button>
                                    ) :(
                                        <Button variant="default" size="icon" className="rounded-2xl w-3/5 bg-primary/40">
                                            <FilePlus className="h-4 w-4" />
                                        </Button>
                                    )}
                                    </TableCell>
                                    <TableCell className="text-center">
                                      <Badge {...getBadgeProps(doc.estado)}>
                                        {doc.estado}
                                      </Badge> 
                                    </TableCell >
                                    <TableCell className="text-center">{doc.ultimaActualizacion}</TableCell>
                                    <TableCell className="space-x-8 text-center">
                                          {doc.verPDF === true ? (
                                            <Button variant="default" size="sm" className="rounded-2xl w-1/4">Ver PDF</Button>
                                          ) : (
